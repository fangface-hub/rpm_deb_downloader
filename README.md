...
誤検知対策としては、Nuitka オプション調整よりも「信頼された証明書での署名」を優先してください。
...
GitHub Actions で誤検知を下げるには、Secrets に `MSIX_SIGN_CERT_BASE64` / `MSIX_SIGN_CERT_PASSWORD` を設定して
信頼された証明書で署名してください。未設定時は self-signed 証明書で署名されます。
