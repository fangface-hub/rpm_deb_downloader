...
class RpmResolver:
    ...
    @staticmethod
    def _find_tool_binary(filename: str) -> str | None:
        app_dir = get_app_dir()
        candidates = [
            app_dir / "tools" / "bin" / filename,
            app_dir / "_internal" / "tools" / "bin" / filename,
    ...
    def _add_rpmmd_repo(self, repo: solv.Repo, primary_xml: bytes) -> None:
        ...
        if not repo2solv:
            local_repo2solv = self._find_tool_binary("repo2solv.exe")
            if local_repo2solv and os.path.exists(local_repo2solv):
                repo2solv = local_repo2solv
        if not rpmmd2solv:
            local_rpmmd2solv = self._find_tool_binary("rpmmd2solv.exe")
            if local_rpmmd2solv and os.path.exists(local_rpmmd2solv):
